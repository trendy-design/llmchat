      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-1 md:gap-3 md:w-[700px] w-full">
        {examplePrompts?.map((example, index) => (
          <motion.div
            ...
            className="flex bg-white dark:bg-zinc-800 flex-col gap-4 items-start text-sm py-2 px-3 md:py-3 md:px-4  border border-black/10 dark:border-white/5 text-zinc-600 dark:text-zinc-400 w-full rounded-2xl hover:bg-zinc-50 dark:hover:bg-black/20 cursor-pointer"
            key={index}
            animate={{
              opacity: 1,
            }}
