import { CompletionMode, deepResearchWorkflow } from './deep';
...
let activeWorkflow: ReturnType<typeof deepResearchWorkflow> | null = null;
...
ctx.addEventListener('message', async (event: MessageEvent) => {
  ...
  try {
    if (type === 'START_WORKFLOW') {
      ...
      const { mode, question, threadId, threadItemId, parentThreadItemId, messages, config, apiKeys: newApiKeys, mcpConfig } = payload;
      ...
      activeWorkflow = deepResearchWorkflow({
        mode,
        question,
        threadId,
        threadItemId,
      ...
      const startTask = mode === CompletionMode.Deep ? 'router' : 'router';
