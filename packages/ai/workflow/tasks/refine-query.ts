...
const ClarificationResponseSchema = z.object({
        ...
        clarifyingQuestion: z.object({
                ...
                type: z.enum(['multiple']).describe("The type of choice the user can make. you can also give multiple choices"),
                options: z.array(z.string()).min(1).max(3).describe("2-3 options for the question")
        }),
        refinedQuery: z.string().describe("Refined query with the same format as the original query but more specific and accurate").optional(),
});
