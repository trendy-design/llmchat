...
import { googleSearchPrompt } from '@repo/shared/config';
  ...
  return new DynamicStructuredTool({
    ...
    func: async ({ input }, runManager) => {
      ...
      try {
        ...
        const googleSearchResult = response?.data?.items?.slice(0, 5)?.map((item: any) => ({
          title: item.title,
...
const googleSearchToolDefinition: ToolDefinition = {
  ...
  validateAvailability: async (context) => {
    return !!(
      context?.preferences?.googleSearchApiKey && context?.preferences?.googleSearchEngineId
    );
  },
