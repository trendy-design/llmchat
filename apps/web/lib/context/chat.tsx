"use client";
import { TChatContext, TChatProvider } from "@repo/shared/types";
...
export const ChatProvider: FC<TChatProvider> = ({ children, sessionId }) => {
  ...
  const setSession = store((state: any) => state.setSession);
  const setCurrentMessage = store((state: any) => state.setCurrentMessage);
  const setMessages = store((state: any) => state.setMessages);
