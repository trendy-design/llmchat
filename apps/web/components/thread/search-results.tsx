...
export const getReaderToolMeta = (
  input: ToolCallType,
  output: ToolCallResultType
): { url: string } => {
  return {
...
export const SearchAndReadingResults = ({
  ...
  const readResults = useMemo(() => {
    ...
    return searchResults.filter(result => result.link && readingResults.includes(result.link));
