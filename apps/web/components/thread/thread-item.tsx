...
export const SearchAndReadingResults = ({ toolCalls, toolCallResults }: SearchAndReadingResultsProps) => {
  ...
  return (
    <Flex direction="col" gap="md" className="w-full">
      ...
      { searchToolResults[0]?.args?.reasoning ? <p className="text-xs text-zinc-500">Reasoning: {searchToolResults[0]?.args?.reasoning as string}</p> : null}
  ...
  return (
    <div className="flex w-full flex-col gap-4">
    {
      Object.entries(block).map(([key, value]) => (
        ...
        <p key={key} className="text-xs text-zinc-500"><pre className="bg-zinc-100 p-2 rounded-md whitespace-pre-wrap"><code>{JSON.stringify(value, null, 2)}</code></pre></p>
        </div>
      ))
    }
    </div>
