...
const DAILY_CREDITS_AUTH = process.env.FREE_CREDITS_LIMIT_REQUESTS_AUTH ? parseInt(process.env.FREE_CREDITS_LIMIT_REQUESTS_AUTH) : 0;

const DAILY_CREDITS_IP = process.env.FREE_CREDITS_LIMIT_REQUESTS_IP ? parseInt(process.env.FREE_CREDITS_LIMIT_REQUESTS_IP) : 0;

// Lua scripts as named constants - these work with Upstash Redis
...
async function getRemainingCreditsForUser(userId: string): Promise<number> {
	...
	try {
		...
		const result = await redis.eval(GET_REMAINING_CREDITS_SCRIPT, [key, lastRefillKey], [DAILY_CREDITS_AUTH.toString(), now]);
...
async function getRemainingCreditsForIp(ip: string): Promise<number> {
	...
	try {
		...
		const result = await redis.eval(GET_REMAINING_CREDITS_SCRIPT, [key, lastRefillKey], [DAILY_CREDITS_IP.toString(), now]);
...
async function deductCreditsFromUser(userId: string, cost: number): Promise<boolean> {
	try {
		...
		const result = await redis.eval(DEDUCT_CREDITS_SCRIPT, [key], [cost.toString()]);
